...
exports.componentTransformer = ({ templateProcessor, stylesheetProcessor, sourceFileWriter }) => (context) => (sourceFile) => {
    ...
    const visitComponents = (node) => {
        if (exports.isTemplateUrl(node)) {
            ...
            if (typeof template === 'string') {
                const synthesizedNode = ts.updatePropertyAssignment(node, ts.createIdentifier('template'), ts.createLiteral(template));
                if (sourceFileWriter) {
        ...
        else if (exports.isStyleUrls(node)) {
    ...
    const visitDecorators = (node) => exports.isComponentDecorator(node)
...
//# sourceMappingURL=ts-transformers.js.map